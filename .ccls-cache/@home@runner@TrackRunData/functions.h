...
//Checks Availability of username reading the user.txt file comparing the username in the file to the provided username 
//boolean because its either taken or not 
bool usernameAvailability(const usr &currentUsr) {
  string line, existingUser;
  ifstream inputFile("user.txt"); //REads user.txt file which stores all user info such as password and username 
  if (inputFile.is_open()) {

    while (getline(inputFile, line)) { //Gets allthe line from the user.txt file 
      stringstream ss(line); 
      //https://www.geeksforgeeks.org/stringstream-c-applications/
      //stringstream allows me to extract the data and compare easily
      //extracting user.txt info using stringstream 
      ss >> existingUser;
      //if these existingUser which we collect in this loop ever matches with the username that the user provided matches it will return false not allowing them to create an account w the same username 
...
void summary(int year, int month) {
  ...
  if (inputFile.is_open()) {
    while (getline(inputFile, line)) {
      ...
      runYear = stoi(date.substr(6, 4)); //sub string of 6,4 represents the year 
  ...
  while (option == 'y' || option == 'Y') {
    ...
    case 1: {
      ...
      while (totalDistance < 1500) {
        ...
        lapTimeSec -= lapTimeHr * 3600; //subtracting the seconds accounted for hour 
        int lapTimeMin = lapTimeSec / 60; //same thing for the min same as the the 2 lines  above this  
        lapTimeSec -= lapTimeMin * 60; // subtracting the seconds accounted for min 
        ...
        totalDistance += distanceLap; //adds up the total distance by distance per lap 
      }
      ...
      cout << "\n\nCalculate Again? (y/n): ";
      cin >> option;
      totalDistance = 0; //resets the total distance to perform the calculation again if they chose so 
      break;
    }
    case 2: {
      // Get user input
...
void mainMenu() {
  ...
  while (true) {
    ...
    case 2: {
      ...
      cin >> month;
      summary(year, month);
      //calling function and passing variables year and month to perform calculation 
      break;
    }
    case 3: {
      //if 3 
